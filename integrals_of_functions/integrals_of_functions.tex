...
\begin{note}
The rational functions that we will deal with in this section include
\begin{itemize}
...
\stackunder{%
  x-1 \stackon[1pt]{\showdiv{x^{3} + \hspace{10pt} + x}}{x^{2} + x + 2}%
}{%
  \Shortstack[l]{{\underline{x^{3}-x^{2}}\ph{+ x}} \ph{x^{3}-}x^{2}+x {\ph{x^{3}-}\underline{x^{2}-x}} \ph{12}34 {\ph{12}\underline{30}} %
...
\end{align*}
Our goal is to rewrite this rational function in one of the four forms mentioned in Note \ref{four_cases_for_partial_fractions}
\end{example}
