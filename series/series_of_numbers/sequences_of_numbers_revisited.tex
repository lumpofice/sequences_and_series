...
\begin{recall}
    From Theorem \ref{geometric_term_sequence}, we discovered $\{r^{n}\}_{n=1}^{\infty}$ is convergent for $r \in [-1, 1]$. Particularly, we discovered
    \begin{align*}
        \lim_{n \longrightarrow \infty} r^{n} = 0 \hspace{20pt} \lvert r \rvert < 1
    \end{align*}
    We will show that $\sum_{n = 1}^{\infty}{r^{n}} < \infty$, when $\lvert r \rvert < 1$. But first, a couple of things.
\end{recall}
...
\end{align*}
Push $m \longrightarrow \infty$. If $\lvert \varphi \rvert < 1$, $\lim_{m \longrightarrow \infty} \varphi^{m+1} = 0$, by Theorem \ref{geometric_term_sequence}.\\
If $\lvert \varphi \rvert = 1$, then $S_m = \sum_{j = 1}^{\infty}{r^{j}} = c + c + \cdots + c = c \cdot m$ and
\begin{align*}
    \lim_{m \longrightarrow \infty} S_{m} = \lim_{m \longrightarrow \infty} c \cdot m = c \cdot \infty = \infty
\end{align*}
Obviously, when $\lvert \varphi \rvert > 1$, we have $S_{m} > c \cdot m$ and
\begin{align*}
    S_{m} \longrightarrow \infty \hspace{4pt} \text{as} \hspace{4pt} m \longrightarrow \infty
\end{align*}
\end{proof}
\end{theorem}
